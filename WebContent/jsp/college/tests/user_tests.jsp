...
<meta charset="utf-8" />
<title>Tests | MySuperBrain.com</title>
				...
				<div class="page-content">
					...
					<div class="page-content-area">
						<div class="page-header">
							<h1>
								Student Tests<small> <i
									class="ace-icon fa fa-angle-double-right"></i> Click on take test to proceed
								</small>
							</h1>
						</div>
						<!-- /.page-header -->
								   ...
								   <c:forEach items="${quizList}" var="quiz" varStatus="itr">
								    <tr>
								     ...
								     <td>${quiz.name }</td>
								       <td>${quiz.noq }</td>
								         <td>${quiz.duration }</td>
								     <td><a href="${pageContext.servletContext.contextPath}/user/taketest/${quiz.quiz_id}" class="btn btn-primary btn-sm"> Take Test</a></td>
								    </tr>
								   </c:forEach>
								  </table>
								 <div align="right">
								  <tag:paginate max="10" offset="${offset}" count="${totalQuizCount}"
								   uri="${pageContext.servletContext.contextPath}/tests/${client}/${usersegment}/${categoryid}" next="Next" previous="Previous" />
							</div>
							</c:when>
							<c:otherwise>
