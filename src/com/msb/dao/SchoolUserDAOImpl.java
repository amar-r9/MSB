...
public class SchoolUserDAOImpl implements SchoolUserDAO {
	...
	public boolean isFan(String username, String fan, String usertype) {
		...
		try {
			...
			Criterion fan_user = Restrictions.eq("fan", fan);
			
			Criterion user_type = Restrictions.eq("user_type", usertype);

			criteria.add(Restrictions.and(user, fan_user, user_type));

			FanTable isFan = (FanTable) criteria.uniqueResult();
	...
	public boolean updateUserFanCount(String username) {
		...
		try {
			...
			Query updateQuery = session
					.createSQLQuery("update SCHOOL_USER set no_of_fans = no_of_fans +1 "
	...
	public boolean updateCountInFanCountTable(String username) {
		...
		try {
			...
			Query updateQuery = session
					.createSQLQuery("UPDATE FAN_COUNT SET COUNT = COUNT+1 WHERE USER_NAME = :username");
			updateQuery.setParameter("username", username);
			int updated = updateQuery.executeUpdate();
	...
	public List<UploadATalent> getUserUATEntries(String username, int limit) {
		...
		try {
			...
			criteria.setFetchMode("competition", FetchMode.JOIN);
			List<UploadATalent> pmdCompList = criteria.list();
