...
public class QuizDAOImpl implements QuizDAO {
	...
	public Quiz getQuizByName(String id) {
		...
		try {
			Criteria criteria = session.createCriteria(Quiz.class);
			Criterion category = Restrictions.eq("id", id);
			criteria.add(Restrictions.and(category));
	...
	@Override
	public List<Quiz> getQuizzesForClient(String client) {
	...
	@Override
	public QuizScores getQuizResult(String id, String username) {
		Session session = sessionFactory.getCurrentSession();

		try {
			Criteria criteria = session.createCriteria(QuizScores.class);
			Criterion quizId = Restrictions.eq("quiz_id", id);
			Criterion userName = Restrictions.eq("user_name", username);
			criteria.add(Restrictions.and(quizId, userName));
			
			QuizScores quizScore = (QuizScores) criteria
					.uniqueResult();
			return quizScore;
		} catch (HibernateException e) {
			e.printStackTrace();
	...
	@Override
	public String getStudentName(String mail) {
		Session session = sessionFactory.getCurrentSession();
		...
		try {
			...
			//String name = (SchoolUser) criteria.uniqueResult();
