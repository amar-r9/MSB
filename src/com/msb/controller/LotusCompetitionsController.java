...
@Controller
public class LotusCompetitionsController {
	...
	@RequestMapping(value = "/lotus/competitions", method = RequestMethod.GET)
	public String competitions() {
		return "/lotus/competitions/competitions";
	}

	@RequestMapping(value = "/lotus/competition_list", method = RequestMethod.GET)
	public String competitionsList() {
		return "/lotus/competitions/competition_list";
	}
	
	@RequestMapping(value = "/lotus/{competition_type}/competition-list", method = RequestMethod.GET)
	public ModelAndView getCompetitions(
		...
		if (competitions != null && competitions.size() > 0) {
			...
			modelAndView.setViewName("/lotus/competitions/competition_list");
			return modelAndView;
		} else{
			
				return new ModelAndView("redirect:/lotus/competition_list?success=false&cat="+competition_type+"");
		}
			

	}

	@RequestMapping(value = "/lotus/competitions/{client}", method = RequestMethod.GET)
	public ModelAndView competitions(
			@PathVariable(value = "client") String client) {
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.addObject("client", client);
		
		modelAndView.setViewName("/lotus/competitions/competitions");
		return modelAndView;

	}

	@RequestMapping(value = "/lotus/user/mst_competition_saved", method = RequestMethod.GET)
	public String paintingCompetitionSaved() {
		return "/lotus/competitions/mst_competition_saved";
	}

	@RequestMapping(value = "/lotus/user/text-competition-saved", method = RequestMethod.GET)
	public String textCompetitionSaved() {
		return "/lotus/competitions/text_competition_saved";
	}

	@RequestMapping(value = "/lotus/user/upload_a_talent_saved", method = RequestMethod.GET)
	public String wiytCompetitionSaved() {
		return "/lotus/competitions/upload_a_talent_saved";
	}

	@RequestMapping(value = "/lotus/user/{type}/text_competition/{id}", method = {
	...
	public ModelAndView competitionsSubmissionForm(
		...
		if (competition != null) {
			...
			modelAndView.setViewName("/lotus/competitions/text_competition");
			return modelAndView;
		} else
			return new ModelAndView("redirect:/competitions?success=false");
	}

	@RequestMapping(value = "/lotus/user/{type}/competition/{id}", method = {
			RequestMethod.POST, RequestMethod.GET })
	public ModelAndView mstCompetitionsSubmissionForm(
		...
		if (competition != null) {
			...
			modelAndView.addObject("type", type);
			modelAndView.addObject("competition", competition);
			modelAndView.setViewName("/lotus/competitions/mst_competition");
			return modelAndView;
		} else
			return new ModelAndView("redirect:/competitions?success=false");
	}
	...
	/*
	 * @RequestMapping(value = "/lotus/user/mst_competition/dance", method = {
	...
	@RequestMapping(value = "/lotus/user/upload_a_talent", method = {
			RequestMethod.POST, RequestMethod.GET })
	public ModelAndView uatSubmissionForm(
		...
		modelAndView.setViewName("/lotus/competitions/upload_a_talent");
		return modelAndView;

	}

	@RequestMapping(value = "/lotus/user/submit-text-competition", method = RequestMethod.POST)
		...
		if (!result.hasErrors()) {
			...
			//if (user_total_competition_submissions >= 3)
				//return new ModelAndView(
							//"redirect:/lotus/user/text-competition-saved?"+competitionText.getEntry_type()+"&success=false&exceeded=true");
			//else {
				...
				if (id != 0) {
					...
					return new ModelAndView(
							"redirect:/lotus/user/text-competition-saved?type="+competitionText.getEntry_type()+"&success=true&id="
									+ id);
				} else
					return new ModelAndView(
							"redirect:/lotus/user/text-competition-saved?type="+competitionText.getEntry_type()+"&success=false");
			//}
		} else {
			return new ModelAndView("redirect:/lotus/user/text_competition/"
					+ competitionText.getEntry_type()
	...
	@RequestMapping(value = "/lotus/user/submit-mst-competition", method = RequestMethod.POST)
	public ModelAndView submitSinging(
			...
			if (id != 0) {
				...
				//mailService.sendMail(subject, body, userData.getMail(), false, "Submitted Your Entry Successfully");
				return new ModelAndView(
						"redirect:/lotus/user/mst_competition_saved?type="+mst.getSubmission_type()+"&success=true&id="
								+ id);
			} else
				return new ModelAndView(
						"redirect:/lotus/user/mst_competition_saved?type="+mst.getSubmission_type()+"&success=false");
		

	}

	@RequestMapping(value = "/lotus/user/submit-uat-competition", method = RequestMethod.POST)
	public ModelAndView submitUploadATalent(
		...
		if (uat.getEntry_type().equalsIgnoreCase("text")) {
			...
			if (id != 0) {
				...
				otherService.updateUserDetails(userData);
				return new ModelAndView(
						"redirect:/lotus/user/upload_a_talent_saved?success=true&id="
								+ id);
			} else
				return new ModelAndView(
						"redirect:/lotus/user/upload_a_talent_saved?success=false");
		} else {
	 ...
	 * modelAndView.addObject("competition", competition);
	 * 
	 * modelAndView.setViewName("/lotus/competitions/competition_info");
	 * 
	 * return modelAndView; } else
	 * 
	 * return new ModelAndView("redirect:/lotus/competitions?success=false");
	 * 
	 * }
	 */
